...
- 2026-09-01T06:10:00Z, task 6dfb5708 review-round-2 fix pass: re-pointed
  ...
  stale (`engine.ts:356-400` -> `384-460` for `runHealthCheck`,
  `engine.ts:383-387` -> `428-432` for the `docker compose exec ... node -e`
  `runExec` call, `engine.ts:381` -> `420` for the trust-boundary comment
  above it), against this commit's own further `engine.ts` edits (added
  `STEP_OUTPUT_MAX_CHARS`/`capStepOutput` before `runStep`, removed
  `MAX_BUFFER_MB`, fixed `stepExecOptions`'s truthiness check). Also
  re-pointed every `engine.ts` citation in `deploy-phase-model.md` that the
  entry below got right (each shifts by -6 lines from this commit's own
  edits above `stepExecOptions`): `engine.ts:111-116` -> `105-110`,
  `211-213` -> `205-207`, `63-72` -> `57-66`, `118-126` -> `112-120`,
  `104` -> `98`, `197/249/259/270/279` -> `191/243/253/264/273`,
  `294` -> `288`, `358/368` -> `352/362`, `305` -> `299`.
  ...
  baseline mismatches untouched, same as the entry below — `engine.ts:408`
  and `engine.ts:458-464`, both out of this task's scope. Also noticed, but
  left untouched for the same reason (predates this task, `apps.ts` is not
  part of this change): `path-containment-idiom.md`'s `apps.ts:28-47` for
...
- 2026-09-01T05:20:00Z, task 6dfb5708 doc pass, at a83e802 head: re-anchored
  ...
  `deploy-phase-model.md` untouched, both already stale before `1f58a19` from
  unrelated later commits and out of this task's scope: `engine.ts:408`
  (the `rollback: boolean` read is actually at `engine.ts:563` at this
  commit's head) and `engine.ts:458-464` (that span is
  `captureContainerLogsTail`'s doc comment at this commit's head, not
  `rollbackIfEnabled`'s call site). Left `exec-trust-boundary.md`'s
  `engine.ts:356-400`/`383-387`/`381` citations (the `runHealthCheck`
  section) unchanged by mistake instead of re-pointing them to this commit's
  actual head locations (`390-466`/`434-438`/`426`); corrected by the entry
  above. Ran `okf-kit@0.3.1 check --json docs/okf` against this commit and
  against `origin/main` (1277ddb): 8 `sources-fresh` warnings / 0 errors at
  this commit vs 14 warnings / 0 errors at `origin/main` — no new finding
  introduced, all remaining warnings pre-existing `sources-fresh` staleness
  in `deploy-failure-surfaces.md` and `health-check-reality.md`.
